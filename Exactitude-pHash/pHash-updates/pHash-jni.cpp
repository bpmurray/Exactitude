...
jfieldID textHash_hash = NULL;
jfieldID textHash_index = NULL;
  ...
  (JNIEnv *e, jclass cl)
{
	...
	vidClass = (jclass)e->NewGlobalRef(e->FindClass("org/phash/VideoHash"));
        textClass = (jclass)e->NewGlobalRef(e->FindClass("org/phash/TextHash"));
	...
	vidHash_hash = e->GetFieldID(vidClass, "hash", "[J");
        textHash_hash = e->GetFieldID(textClass, "hash", "[J");
	textHash_index = e->GetFieldID(textClass, "index", "[I");
	...
	audioCtor = e->GetMethodID(audioClass, "<init>", "()V");
        textCtor = e->GetMethodID(textClass, "<init>", "()V");
...
(JNIEnv *e, jclass cl, jstring f)
{
	const char *file = e->GetStringUTFChars(f, 0);
	int len;
	TxtHashPoint *hash = ph_texthash(file, &len);
   if (!hash) {
      return NULL; // File not found
   }
	jobject textHash = e->NewObject(textClass, textCtor);
	e->SetObjectField(textHash, hash_filename, f);

	// Load the hash & index values
	jlongArray hashVals = e->NewLongArray(len);
	jintArray indexVals = e->NewIntArray(len);
	for (int iX = 0; iX < len; iX++) {
		e->SetLongArrayRegion(hashVals, iX, 1, (jlong *) &hash[iX].hash);
		e->SetIntArrayRegion(indexVals, iX, 1, (jint *) &hash[iX].index);
	}
	e->SetObjectField(textHash, textHash_hash, hashVals);
	e->SetObjectField(textHash, textHash_index, indexVals);
...
JNIEXPORT jdouble JNICALL Java_org_phash_pHash_textDistance
(JNIEnv *e, jclass cl, jobject textHash1, jobject textHash2)
{
   double sim = 1.0; // Default = no match

   // If both hashes are missing, they have to be equal, but
   // if one hash is missing, they don't match
	if (textHash1 == NULL) {
		return (textHash2 == NULL) ? (jdouble) 0.0 : (jdouble) -1.0;
	}

   // Pick up the current values in the objects
	jlongArray hash1  = (jlongArray) e->GetObjectField(textHash1, textHash_hash);
	jintArray  index1 = (jintArray) e->GetObjectField(textHash1, textHash_index);
	jint       len1   = e->GetArrayLength(hash1);

	jlongArray hash2  = (jlongArray) e->GetObjectField(textHash2, textHash_hash);
	jintArray  index2 = (jintArray) e->GetObjectField(textHash2, textHash_index);
	jint       len2   = e->GetArrayLength(hash2);

	if (len1 <= 0) {
		return (len2<=0) ? (jdouble) 0.0 : (jdouble) -1.0;
	}

   // Convert first arrays to TxtHashPoints
	ulong64  *pHash1  = (ulong64 *)  e->GetLongArrayElements(hash1, 0);
	uint32_t *pIndex1 = (uint32_t *) e->GetIntArrayElements(index1, 0);

	TxtHashPoint *txthash1 = new TxtHashPoint[len1];
	for (int iX = 0; iX < len1; iX++) {
		txthash1[iX].hash  = pHash1[iX];
		txthash1[iX].index = pIndex1[iX];
	}

	e->ReleaseLongArrayElements(hash1, (jlong*) pHash1, 0);
	e->ReleaseIntArrayElements(index1, (jint*) pIndex1, 0);

   // Convert second arrays to TxtHashPoints
	ulong64 *pHash2   = (ulong64 *)  e->GetLongArrayElements(hash2, 0);
	uint32_t *pIndex2 = (uint32_t *) e->GetIntArrayElements(index2, 0);

	TxtHashPoint *txthash2 = new TxtHashPoint[len2];
	for (int iX = 0; iX < len2; iX++) {
		txthash2[iX].hash  = pHash2[iX];
		txthash2[iX].index = pIndex2[iX];
	}

	e->ReleaseLongArrayElements(hash2, (jlong*) pHash2, 0);
	e->ReleaseIntArrayElements(index2, (jint*) pIndex2, 0);

   // Compare the hash points
   int matchLen = 0;
	TxtMatch* matches = ph_compare_text_hashes(txthash1, len1, txthash2, len2, &matchLen);
   sim = (len1-matchLen) / len1; // Ratio of not matched against total
