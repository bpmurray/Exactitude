...
public class pHash
{
	...
	public VideoHash getVideoHash(String file) {
		return videoHash(file);
	}

	public AudioHash getAudioHash(String file) {
		return audioHash(file);
	}

	public DCTImageHash getDctImageHash(String file) {
		return dctImageHash(file);
	}

	public MHImageHash getMhImageHash(String file) {
		return mhImageHash(file);
	}

	public TextHash getTextHash(String file) {
		return textHash(file);
	}

	// Compare hash values
	public double getImageDistance(ImageHash hash1, ImageHash hash2) {
		return imageDistance(hash1, hash2);
	}

	public double getAudioDistance(AudioHash hash1, AudioHash hash2) {
		return audioDistance(hash1, hash2);
	}
	
	public double getVideoDistance(VideoHash hash1, VideoHash hash2, int threshold) {
		return videoDistance(hash1, hash2, threshold);
	}
	
	public double getVideoDistance(VideoHash hash1, VideoHash hash2) {
		return getVideoDistance(hash1, hash2, DEFAULT_THRESHOLD);
	}

	public int getTextDistance(TextHash txtHash1, TextHash txtHash2) {
		return textDistance(txtHash1, txtHash2);
	}
